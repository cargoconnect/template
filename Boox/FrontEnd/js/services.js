...
.factory('Notification', function () {
    ...
    .factory('SendPost', function ($http) {
        return function (obj) {
            $http.post('/api/account/post', obj);
        }
    })
    ...
    .factory('HomeService', 
        ...
        function ($http) {
            ...
            service.GetSecureData = function (callback) {
                // must authorized before call web api 
                $http.get('/api/account/get')
                    .success(function (response) {
                        callback(response);
                    });
    ...
    .factory('AuthenticationService', 
        ...
        function (Base64, $http, $cookieStore, $rootScope) {
            ...
            service.Login = function (username, password, callback) {
                $http.post('/api/account/login', '\'{"email" : "'+username+'","password" : "'+password+'"}\'')
                    .success(function (response) {
                        callback(response);
                    });
            };
